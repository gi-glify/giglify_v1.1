...
export default function PaymentProviderTabs({ selectedValue, onChange }) {
    ...
    return (_jsx("div", { className: "grid grid-cols-3 gap-1 rounded-xl border border-[var(--border)] p-1 bg-black/[.02] dark:bg-white/[.03]", role: "tablist", "aria-label": "Payment provider", children: PAYMENT_PROVIDERS.map((provider, index) => (_jsx("button", { id: `payment-provider-tab-${provider.id}`, type: "button", role: "tab", "aria-selected": selectedValue === provider.id, "aria-controls": `payment-provider-panel-${provider.id}`, tabIndex: selectedValue === provider.id ? 0 : -1, onClick: () => onChange(provider.id), onKeyDown: (event) => handleKeyDown(event, index), className: `
