...
export async function fetchTasks(): Promise<{ tasks: TaskCatalogItem[]; error: Error | null }> {
  ...
  if (!primary.error) {
    return { tasks: (primary.data ?? []).map((row) => mapTaskRow(row as TaskRow)), error: null };
  }
  ...
  if (!withoutType.error) {
    return { tasks: (withoutType.data ?? []).map((row) => mapTaskRow(row as TaskRow)), error: null };
  }
...
export async function validateMcqAnswer(input: { taskCode: string; questionNumber: number; answer: string }) {
  ...
  if (!data || typeof data.correct !== 'boolean') throw new Error('The answer validator returned an invalid response.');
