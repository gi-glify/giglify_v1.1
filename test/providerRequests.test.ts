...
test("builds a Palpluss STK request only when KES and phone are supplied", () => {
...
test("rejects Palpluss requests without an explicit KES amount", () => {
  assert.throws(
    () => buildPackageProviderRequest({ ...base, provider: "palpluss", phone: "254712345678" }, { secret: "palpluss-key" }),
    /Palpluss requires amountKes and phone/,
  );
});
