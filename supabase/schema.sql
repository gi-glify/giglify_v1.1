...
returns trigger as $$
begin
  if auth.uid() = old.id and (new.profile_edit_count is distinct from old.profile_edit_count or new.profile_edit_appeal_approved is distinct from old.profile_edit_appeal_approved) then
...
create trigger enforce_profile_edit_limit before update on public.profiles for each row execute procedure public.enforce_profile_edit_limit();
